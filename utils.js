...
async function getAvatarUrl(input) {
	try {
		if (!input) throw new Error("No input provided");
		let userID = input.startsWith('https://') ? await findUid(input) : input?.trim();
			...
			if (response.headers['content-type']?.startsWith('image/')) {
				...
				const response = await axios.get('https://m.facebook.com/profile.php', {
				params: {
					'id': userID,
					'm_redirect': '0',
					'locale': 'en_US',
					'hpir': '1',
					'http_ref': 'eyJ0cyI6MTc2NTA5MTE5MzAwMCwiciI6IiJ9'
				},
				headers: {
					'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
					'accept-language': 'en-US',
					'cookie': 'datr=zuhsaE2t2s_uxYp9y_es01op; sb=z-hsaMh-n04IVYEm9TmZwpeY; ps_l=1; ps_n=1; vpd=v1%3B805x400x2.706249952316284; locale=en_US; dpr=2.706249952316284; m_pixel_ratio=2.706249952316284; wd=400x805; fr=0ZkwhDQ2lWcBuYvZv..BpD2k3..AAA.0.0.BpNSd5.AWcCOnSQ6se9YnjJX107cXStYZo',
					'dpr': '2.706249952316284',
					'priority': 'u=0, i',
					'referer': 'https://cfffe4ff-d7c9-4bba-93a9-8b905ba0015b-00-18lclz4gju81t.pike.replit.dev/',
			...
			const $ = cheerio.load(response.data);
				let profilePhoto = '';
				const profilePhotoDiv = $('div[aria-label*="Profile photo"]');
			if (profilePhotoDiv.length) {
				const profileImg = profilePhotoDiv.find('img[src*="https://"]').last();
				if (profileImg.length) {
					profilePhoto = profileImg.attr('src')?.replace(/&amp;/g, '&') || '';
				}
			}else {
				profilePhoto = ($('meta[property="og:image"]').attr('content') || $('meta[property="twitter:image"]').attr('content')) ?? null;
			}
				return profilePhoto;
			}
		} catch (error) {
...
async function getName(input) {
  try {
    ...
    if (/^\d+$/.test(url)) {
      ...
      followRedirect = true;
    } else if (url.startsWith("profile.php")) {
      url = `https://www.facebook.com/${url}`;
      followRedirect = true;
    } else if (/^https?:\/\/(www\.)?facebook\.com\//i.test(url)) {
      if (url.includes("profile.php?id=")) followRedirect = true;
    } else {
      throw new Error("Invalid Facebook input");
    }
